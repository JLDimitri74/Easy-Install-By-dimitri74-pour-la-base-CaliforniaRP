...
import io
def usage():
	print('Utilisation: python FindAndReplace.py [Ancienne chaîne] [Nouvelle chaîne]' \
		  '[Filtres de fichiers (par défaut: ". .txt' '.html' '.erb' '.lua' '.cfg' '.json")] [Répertoire à vérifier(.)]')
...
def main():
	...
	try:
		...
		if len(sys.argv) < 4:
			patterns = ['.txt', '.html', '.erb', '.lua', '.cfg', '.json', '.sql']
		else:
